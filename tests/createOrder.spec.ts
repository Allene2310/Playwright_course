...
test('create an order', async ({ page }) => {
  ...
  await page.getByText('В кошик').or(page.locator('#button-cart')).click();
  ...
  await page
    ...
    .getByRole('heading', { name: 'iPhone' }) 
    .getByRole('link')
    .click();
  await page.getByRole('button').and(page.locator('#button-cart')).click();
  //await page.pause(); 
  //await page.getByRole('button', { name: 'В кошик' }).click();
  //await page.getByRole('link', { name: ' Кошик' }).click();
